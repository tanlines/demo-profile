...
; Global variables
global overlayGui := ""
global overlayWidth := 300
global overlayHeight := 300
...
; Create the main GUI
Gui, Main:New, +AlwaysOnTop +ToolWindow, Screenshot Overlay
Gui, Main:Add, Text,, Width:
Gui, Main:Add, Edit, vWidthInput w80, 300
Gui, Main:Add, Text,, Height:
Gui, Main:Add, Edit, vHeightInput w80, 300
Gui, Main:Add, Text,, Mode:
Gui, Main:Add, DropDownList, vModeInput w80 gModeChanged, square|vertical
Gui, Main:Add, Checkbox, vConvertWebP Checked, Convert to WebP
Gui, Main:Add, Button, w120 gToggleOverlay, Show Overlay (Ctrl+Shift+S)
Gui, Main:Add, Text,, Instructions:
Gui, Main:Add, Text,, 1. Set width/height
Gui, Main:Add, Text,, 2. Press Ctrl+Shift+S to toggle overlay
Gui, Main:Add, Text,, 3. Press Ctrl+Shift+D to copy screenshot to clipboard
Gui, Main:Show, x100 y100

return

; Function to handle mode dropdown changes
ModeChanged:
    Gui, Main: Submit, Nohide
    if (ModeInput = "square") {
        GuiControl, Main:, WidthInput, 300
        GuiControl, Main:, HeightInput, 300
    } else if (ModeInput = "vertical") {
        GuiControl, Main:, WidthInput, 600
        GuiControl, Main:, HeightInput, 1000
    }
return

hover:
Gui, Main: Submit, Nohide
overlayWidth:=WidthInput
overlayHeight:=HeightInput
xd := WidthInput
if (display) {
MouseGetPos, currx, curry
overlay_rect(currx, curry, overlayWidth, overlayHeight, 3, "Red", False)
}
return

; Hotkey to show overlay
^+s::
Gui, Main: Submit, Nohide
overlayWidth:=WidthInput
overlayHeight:=HeightInput
...
; Function to update overlay position to follow mouse
UpdateOverlayPosition() {
    global overlayWidth, overlayHeight, overlayX, overlayY, overlayGui, isOverlayVisible
    if (!isOverlayVisible || !overlayGui) {
        return
    }
    ...
    ; Update global overlay position variables
    overlayX := cursorX
    overlayY := cursorY
    ...
    ; Create new overlay using overlay_rect function
    overlayGui := overlay_rect(cursorX, cursorY, overlayWidth, overlayHeight, 3, "Red")
}
^+d::
Gui, Main: Submit, Nohide
overlayWidth:=WidthInput
...
TakeScreenshot(clickX-overlayWidth/2, clickY-overlayHeight/2, overlayWidth, overlayHeight)
return

; Function to take screenshot and save as PNG, optionally convert to WebP
TakeScreenshot(x, y, width, height) {
    global ConvertWebP
    ; Small delay to ensure overlay is hidden
    Sleep, 100
    
    ; Use the CaptureScreen function with coordinates
    ; Format: "Left, Top, Right, Bottom" where Right = Left + Width, Bottom = Top + Height
    right := x + width
    bottom := y + height
    captureCoords := x . ", " . y . ", " . right . ", " . bottom
    
    ; Generate filename with timestamp
    FormatTime, timestamp, , yyyyMMdd_HHmmss
    filename := A_ScriptDir . "\screenshot_" . timestamp . ".png"
    
    ; Capture the screen area and save as PNG
    CaptureScreen(captureCoords, False, filename)

    ; Optionally convert to WebP using cwebp
    Gui, Main: Submit, Nohide
    if (ConvertWebP) {
        webpFile := A_ScriptDir . "\screenshot_" . timestamp . ".webp"
        RunWait, %ComSpec% /c cwebp -q 85 "%filename%" -o "%webpFile%",, Hide
        if (ErrorLevel = 0) {
            FileDelete, %filename%
            ToolTip, WebP saved!
        } else {
            ToolTip, PNG saved. WebP conversion failed.
        }
    } else {
        ; Show success notification
        ToolTip, Screenshot saved as PNG!
    }
